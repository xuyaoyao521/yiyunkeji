...
var img = {
	...
	errorhandle : function(obj, is_err_t) {
		...
		parentnode.append('<div class="loading" style="background:url('+ IMGDIR +'/imageloading.gif) no-repeat center center;background-size:35px;width:'+parentnode.width()+'px;height:'+parentnode.height()+'px"></div>');
...
var popup = {
	...
	open : function(pop, type, url) {
		...
		if(POPMENU[pop.attr('id')]) {
			...
			$(".dialogbox").find(".btn_default").css({"margin":9});
		}
...
var dialog = {
	init : function() {
		$(document).on('click', '.dialog', function() {
			...
			popup.open('<img src="' + IMGDIR + '/imageloading.gif" width="35">');
			$.ajax({
				type : 'GET',
