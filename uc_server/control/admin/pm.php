...
class control extends adminbase {
	...
	function onls() {
		...
		if($this->submitcheck() || getgpc('searchpmsubmit', 'G')) {
			...
			if($count) {
				...
				$multipage = $this->page($count, UC_PPP, $page, 'eyun_admins.php?m=pm&a=ls&srchtablename='.$srchtablename.'&srchauthor='.urlencode($srchauthor).'&srchstarttime='.urlencode($srchstarttime).'&srchendtime='.urlencode($srchendtime).'&srchmessage='.urlencode($srchmessage).'&searchpmsubmit=true');
			}
		}
	...
	function ondelete() {
		...
		if($this->submitcheck()) {
			...
			if(empty($pmids)) {
				$this->message('pm_delete_noselect', 'eyun_admins.php?m=pm&a=ls&srchtablename='.$srchtablename.'&srchauthor='.urlencode($srchauthor).'&srchstarttime='.urlencode($srchstarttime).'&srchendtime='.urlencode($srchendtime).'&srchmessage='.urlencode($srchmessage).'&searchpmsubmit=true');
			}
			...
			$this->message('pm_clear_succeed', 'eyun_admins.php?m=pm&a=ls&srchtablename='.$srchtablename.'&srchauthor='.urlencode($srchauthor).'&srchstarttime='.urlencode($srchstarttime).'&srchendtime='.urlencode($srchendtime).'&srchmessage='.urlencode($srchmessage).'&searchpmsubmit=true');
		}
	}
	...
	function onclear() {
		...
		if($this->submitcheck() || getgpc('clearpmsubmit', 'G')) {
			...
			$nexturl = "eyun_admins.php?m=pm&a=clear&usernames=$usernames&current=$next&pertask=$pertask&clearpmsubmit=1";
			...
			if($usernames) {
				...
				if($processed) {
					...
					$this->message('pm_clear_succeed', 'eyun_admins.php?m=pm&a=clear');
				}
			}
		}
