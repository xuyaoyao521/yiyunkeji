...
class control extends adminbase {
	...
	function onsync() {
			...
			if($app = $this->cache['apps'][$appid]) {
				...
				if(!$testrelease) {
					if(!($data = $this->sync_unserialize($data, ''))) {
						header('location: '.UC_API.'/eyun_admins.php?m=credit&a=sync&step=0&stepapp='.$stepapp.'&testrelease=1&sid='.$this->view->sid);
						exit();
					} else {
						$stepapp++;
					}
				...
				header('location: '.UC_API.'/eyun_admins.php?m=credit&a=sync&step=0&stepapp='.$stepapp.'&sid='.$this->view->sid);
			} else {
				header('location: '.UC_API.'/eyun_admins.php?m=credit&a=sync&step=1&sid='.$this->view->sid);
			}
			exit();
		}
		...
		$this->message('syncappcredits_updated','eyun_admins.php?m=credit&a=ls');
	}
