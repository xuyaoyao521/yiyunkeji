...
class control extends adminbase {
			...
			if(!$can_do_login) {
				...
				if(!seccode::seccode_check($seccodehidden, $seccode)) {
					...
					if($errorcode == 0) {
						...
						if($iframe) {
							header('location: eyun_admins.php?m=frame&a=main&iframe=1'.($this->cookie_status ? '' : '&sid='.$this->view->sid));
							exit;
						} else {
	...
	function onadd() {
		...
		$this->message('user_add_succeed', 'eyun_admins.php?m=user&a=ls');
	}
	...
	function onls() {
		...
		$multipage = $this->page($num, UC_PPP, $_GET['page'], 'eyun_admins.php?m=user&a=ls&srchname='.$srchname.$urladd);
