...
<div class="container">
	...
	<ul class="memlist fixwidth">
		<li><em><?php if($user['allowadminapp'] || $user['isfounder']) { ?><a href="eyun_admins.php?m=app&a=ls">应用总数</a><?php } else { ?>应用总数<?php } ?>:</em><?php echo $apps;?></li>
		<li><em><?php if($user['allowadminuser'] || $user['isfounder']) { ?><a href="eyun_admins.php?m=user&a=ls">用户总数</a><?php } else { ?>用户总数<?php } ?>:</em><?php echo $members;?></li>
		<li><em><?php if($user['allowadminpm'] || $user['isfounder']) { ?><a href="eyun_admins.php?m=pm&a=ls">短消息数</a><?php } else { ?>短消息数<?php } ?>:</em><?php echo $pms;?></li>
		<li><em>好友记录数:</em><?php echo $friends;?></li>
	</ul>
	
	<h3>通知状态</h3>
	<ul class="memlist fixwidth">
		<li><em><?php if($user['allowadminnote'] || $user['isfounder']) { ?><a href="eyun_admins.php?m=note&a=ls">未发送的通知数</a><?php } else { ?>未发送的通知数<?php } ?>:</em><?php echo $notes;?></li>
		<?php if($errornotes) { ?>
			<li><em><?php if($user['allowadminnote'] || $user['isfounder']) { ?><a href="eyun_admins.php?m=note&a=ls">通知失败的应用</a><?php } else { ?>通知失败的应用<?php } ?>:</em>		
	...
	<ul class="memlist fixwidth">
		...
		<li><em>当前数据库统计:</em><?php echo $dbsize;?></li>		
