...
function parseattachone($html, $pid) {
	$aids=  DB::fetch_all("SELECT * FROM " .DB::table('forum_attachment')." where pid=".$pid."");
	$forum_attachtags=array();
	
		foreach($aids as $kkk=>$vvv){
				$forum_attachtags[$vvv['aid']][]=DB::fetch_first("SELECT * FROM " .DB::table('forum_attachment_'.$vvv['tableid'])." where aid=".$vvv['aid']."");
		}
		
		foreach($forum_attachtags as $k=>$v){
				$html= preg_replace("/\[attach\]".$k."\[\/attach\]/i",'<img src="data/attachment/forum/'.$v[0]['attachment'].'" width="'.$v[0]['width'].'">', $html);
		}
		return $html;
}
