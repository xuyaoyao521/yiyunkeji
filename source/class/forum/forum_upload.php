...
class forum_upload {
	...
	function uploadmsg($statusid) {
		...
		if($this->simple == 1) {
			...
			echo 'DISCUZUPLOAD|'.($_GET['type'] == 'image' ? '1' : '0').'|'.$statusid.'|'.$this->aid.'|'.$this->attach['isimage'].'|'.($this->attach['isimage']||$this->attach['ext']=="mp4" ? $this->attach['attachment'] : '').'|'.$this->attach['name'].'|'.$this->error_sizelimit;
		} else {
