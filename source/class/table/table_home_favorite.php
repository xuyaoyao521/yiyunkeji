...
class table_home_favorite extends discuz_table
{
	...
	public function fetch_all_by_uid_idtype2($uid, $idtype, $favid = 0, $start = 0, $limit = 0) {
		$parameter = array($this->_table);
		$wherearr = array();
		$parameter[] = $uid;
		$wherearr[] = "f.uid=".$uid;
		if(!empty($idtype)) {
			...
			if($idtype=='aid'||$idtype=="tid"){
				$menus2 = DB::fetch_all("SELECT fid FROM " .DB::table('forum_forum')." where fup=1");
				$fids2='';
				foreach($menus2 as $k=>$v){
						$fids2.=','.$v['fid'];
				}	
				
				$fids2=ltrim($fids2,",");
				if($idtype=='aid'){
				$wherearr[] = "t.fid in (".$fids2.")";
				}else{
				$wherearr[] = "t.fid not in (".$fids2.")";		
				}
			}
		...
		$wheresql = !empty($wherearr) && is_array($wherearr) ? ' WHERE '.implode(' AND ', $wherearr) : '';
		return DB::fetch_all("SELECT f.* FROM %t f inner join " .DB::table('forum_thread')." t on f.id=t.tid and f.idtype='tid'  $wheresql ORDER BY f.dateline DESC ".DB::limit($start, $limit), $parameter, $this->_pk);
	...
	public function count_by_uid_idtype2($uid, $idtype, $favid = 0) {
		$parameter = array($this->_table);
		$wherearr = array();
		if($favid) {
			$parameter[] = dintval($favid, is_array($favid) ? true : false);
		...
		return DB::result_first("SELECT COUNT(*) FROM %t f inner join " .DB::table('forum_thread')." t on f.id=t.tid and f.idtype='tid'  $wheresql ", $parameter);
	}
