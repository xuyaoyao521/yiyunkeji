...
class table_forum_forum extends discuz_table
{
	...
	public function fetch_all_forum_for_sub_order() {
		return DB::fetch_all("SELECT ff.*, f.fid, f.type, f.status, f.name, f.fup, f.displayorder, f.inheritedmod FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('forum_forumfield')." ff USING(fid) WHERE f.fid=1 or f.fup=1 and f.status<>'3' ORDER BY f.type<>'group', f.displayorder");
	}
	
	public function fetch_all_forum_for_sub_order_for() {
		return DB::fetch_all("SELECT ff.*, f.fid, f.type, f.status, f.name, f.fup, f.displayorder, f.inheritedmod FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('forum_forumfield')." ff USING(fid) WHERE f.fid!=1 and f.fup!=1 and f.status<>'3' ORDER BY f.type<>'group', f.displayorder");
	}
