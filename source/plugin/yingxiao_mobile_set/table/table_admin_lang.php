...
class table_admin_lang extends discuz_table
{
	
	public function __construct() {

		$this->_table = 'yingxiao_mobile_set_lang';
		$this->_pk    = '';
	...
	public function fetch_all() {
		return DB::fetch_all("SELECT * FROM %t ORDER BY langid ASC", array($this->_table));
	}
	
	public function fetch_all_by_langid($langid) {
		return DB::fetch_all("SELECT * FROM %t WHERE langid=%d", array($this->_table, $langid));
	}
	/*
	public function fetch_all_by_sname($sname) {
		return DB::fetch_all("SELECT * FROM %t WHERE sname='" .$sname. "'", array($this->_table));
	}
	*/
	public function delete_by_langid($langid) {
		DB::query("DELETE FROM %t WHERE langid=%d", array($this->_table, $langid));
	}
	
	public function insert($langvar, $langstr) {
		DB::query("INSERT INTO %t(langvar,langstr) VALUES('$langvar', '$langstr')", array($this->_table));
	}
	
	public function update_by_langid($langid, $langstr) {
		DB::query("UPDATE %t SET langstr='$langstr' WHERE langid=%d", array($this->_table, $langid));
	}
	
	/*
	public function update_by_all_isopen($isopen) {
		DB::query("UPDATE %t SET isopen=%d", array($this->_table, $isopen));
	}
	*/
