...
if($operation == 'admin') {
	...
	if(!submitcheck('searchsubmit', 1)) {
		...
		showsetting('选择分类', '', $_GET['perpage'], "<select name='perpage'  id='perpage'  style='float:left'>".forumselect(FALSE, 0, 0, TRUE)."</select> <button onClick='fabu()' id='button_add'>新增</button>");
...
<script>
	function fabu(){
		var id=document.getElementById("perpage").value;
		//location="forum.php?mod=post&action=newthread&isadmin=1&fid="+id;
		window.open("forum.php?mod=post&action=newthread&t=1&fid="+id);
