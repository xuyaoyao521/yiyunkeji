...
EOF;
		...
		if($membernum > 0) {
			...
			if($uids) {
				...
				foreach($allmember as $uid=>$member) {
                    ...
                    $member['mobile']= DB::result_first('SELECT mobile FROM '.DB::table('common_member_profile').' where uid='.$uid);
		...
		if($membernum) {
			showsubtitle(array('', 'username', 'credits', 'posts','phonenum', 'admingroup', 'usergroup', ''));
			echo $members;
...
EOF;
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('members_edit_statistics', '', '', "$lang[posts]($member[posts])&nbsp;
				&nbsp;$lang[setting_styles_viewthread_userinfo_oltime]: $member[oltime]$lang[hourtime]");
...
function shownewsletter() {
	...
	if($_GET['do'] == 'mobile') {
		...
		showsetting('members_newsletter_method', array('notifymembers', array(
			    ...
			    array('notice', $lang['notice'], array('pmextra' => '', 'posttype' => 'none')),
