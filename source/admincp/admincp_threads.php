...
if((!$operation && !$optype) || ($operation == 'group' && empty($optype) ) || ($operation == 'for' && empty($optype))) {
...
EOT;
    ...
    if($_GET['operation']=="for"){
	showsubmenu('列表', array(
    ...
    showsubmenu('列表', array(
		array('newlist', 'threads'.($operation ? '&operation='.$operation : ''), !empty($newlist)),
		array('search', 'threads'.($operation ? '&operation='.$operation : '').'&search=true', empty($newlist)),
        array('添加新主题', 'threadsadd', empty($newlist)),
	));
	...
	if(submitcheck('searchsubmit', 1) || $newlist) {
		...
		if($conditions) {
			...
			if($_GET['detail']) {
				...
				$threadcount = C::t('forum_thread')->count_search_for($conditions);
                }else{
				...
				if($threadcount) {
				 ...
				 $tlist=C::t('forum_thread')->fetch_all_search_for($conditions, 0, $start, $perpage, 'tid', 'DESC', ' FORCE INDEX(PRIMARY) ');
                }else{
                 $tlist=C::t('forum_thread')->fetch_all_search($conditions, 0, $start, $perpage, 'tid', 'DESC', ' FORCE INDEX(PRIMARY) ');
                }
               
                
                
					foreach($tlist as $thread) {
						$fids[] = $thread['fid'];
						if($thread['isgroup']) {
					...
					if($threadlist) {
						foreach($threadlist as $thread) {
							...
							"<a href=\"forum.php?mod=forumdisplay&fid=$thread[fid]\" target=\"_blank\">".(empty($thread['isgroup']) ? $_G['cache']['forums'][$thread[fid]]['name'] : $groupsname[$thread[fid]])."</a>",
								"<a href=\"home.php?mod=auther&uid=$thread[authorid]\" target=\"_blank\">$thread[author]</a>",
								$thread['replies'],
								$thread['views'],
								$thread['lastpost'],
                                "<a href=\"forum.php?mod=post&action=edit&fid=$thread[fid]&tid=$thread[tid]&pid=$thread[pid]\" target=\"_blank\">编辑</a>"
							), TRUE);
						}
					}
		...
		if(!$threadcount) {
			...
			if($_GET['detail']) {
				showsubtitle(array('', 'subject', 'forum', 'author', 'threads_replies', 'threads_views', 'threads_lastpost','操作'));
				echo $threads;
