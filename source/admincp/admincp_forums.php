...
if($operation == 'admin'||$operation == 'for') {
...
<?php
		...
		$query = C::t('forum_forum')->fetch_all_forum_for_sub_order();
			...
			$forumcount = C::t('forum_forum')->fetch_forum_num_for();

			$query = C::t('forum_forum')->fetch_all_forum_for_sub_order_for();
		}
...
function showforum(&$forum, $type = '', $last = '', $toggle = false) {
	...
	if($last == '') {
		...
		$return .= '<input type="text" name="name['.$forum['fid'].']" value="'.dhtmlspecialchars($forum['name']).'" class="txt" />'.
			...
			<td width="220"><input class="checkbox" value="'.$forum['fid'].'" type="checkbox"'.($type != 'group' ? ' chkvalue="g'.$_G['fg'].'" onclick="multiupdate(this, '.$forum['fid'].')"' : ' name="gc'.$_G['fg'].'" onclick="checkAll(\'value\', this.form, \'g'.$_G['fg'].'\', \'gc'.$_G['fg'].'\', 1)"').' />'.
			'<a href="'.ADMINSCRIPT.'?action=forums&operation=edit&fid='.$forum['fid'].'" title="'.cplang('forums_edit_comment').'" class="act">'.cplang('edit').'</a>'.
			
			($type != 'group' ? '<a href="'.ADMINSCRIPT.'?action=forums&operation=copy&source='.$forum['fid'].'" title="'.cplang('forums_copy_comment').'" class="act">'.cplang('forums_copy').'</a>'.'<a href="eyun_admins.php?action=threadadd&fid='.$forum['fid'].'" class="act">'.发表.'</a>' : '').
		...
		if($last == 'lastboard') {
			...
			$return = '</tbody><tr><td></td><td colspan="4"><div>';
			if($_GET['operation'] == 'for'){
			$return.='<a href="###" onclick="addrow(this, 0)" class="addtr">'.cplang('forums_admin_add_category').'</a>';
			}
			$return.='</div></td>'.
