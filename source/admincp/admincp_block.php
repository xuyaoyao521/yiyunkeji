<style>.floattop,.floattopempty{ display:none}</style>
...
SEARCH;
		...
		if($operation=='jscall') {
			...
			showsubtitle(array('block_name',  'block_dateline',  'operation'));
			$multipage = '';
			if(($count = C::t('common_block')->count_by_admincpwhere($wheresql))) {
				foreach(C::t('common_block')->fetch_all_by_admincpwhere($wheresql, $ordersql, $start, $perpage) as $value) {
					if($value['targettplname']) {
					...
					foreach($list as $bid => $value) {
						...
						showtablerow('', '', array(
							...
							!empty($value['dateline']) ? dgmdate($value['dateline']) : cplang('block_dateline_null'),
							 ...
							 ""
							 ."<a href=\"portal.php?mod=portalcp&ac=block&op=data&bid=$value[bid]&from=cp\" target=\"_blank\" onclick=\"showWindow('showblock',this.href);return false;\">操作</a>"
						));
                        ...
                        showtablerow('', '', array(
							...
							"",
							 ...
							 ""
							 ."<a href=\"portal.php?mod=portalcp&ac=block&op=data&bid=102&from=cp\" target=\"_blank\" onclick=\"showWindow('showblock',this.href);return false;\">操作</a>"
						));
