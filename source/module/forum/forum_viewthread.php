...
if(empty($postlist)) {
	...
	foreach($postlist as $pid => $post) {
		$postlist[$pid]['message'] = preg_replace("/\[attach\]\d+\[\/attach\]/i", '', $postlist[$pid]['message']);
			...
			$postlist[$pid]['zanguo'] =DB::result_first("SELECT id FROM " .DB::table('forum_hotreply_user')." where (uid='".$_G['uid']."' or openid='".$openid."') and pid=".$pid." and type=1")?"on":'';	;
...
if(empty($_GET['viewpid'])) {
	...
	if($_GET['from'] == 'portal') {
		...
		$auither_jianjie=DB::result_first("SELECT jianjie FROM " .DB::table('ucenter_members')." where uid=".$post['authorid']);	
		...
		$auither_arc_list = DB::fetch_all( "SELECT t.tid,p.subject FROM ".DB::table( "forum_thread" )." t  INNER JOIN ".DB::table( "forum_post" )." p on p.tid=t.tid and p.first=1 INNER JOIN ".DB::table( "forum_forum" )." f on f.fid=t.fid  WHERE t.displayorder>=0 ".$whereadd." group by t.tid ORDER BY t.dateline DESC LIMIT 5" );
...
$ceo_listcell = isset($settings['ceo_listcell']) ? intval($settings['ceo_listcell']) : 3;
		...
		$tids=DB::fetch_all( "SELECT itemid FROM ".DB::table( "common_tagitem" ) );//." where tagid in (".$tagids.") and itemid!=".$post['tid']
		$tidarray ="";
		 $threadlisttags = array();
		foreach($tids as $result) {
			$tidarray.=",".$result['itemid']; ;
		}
		$tidarray=ltrim($tidarray,",");
		...
		$threadlisttags = DB::fetch_all( "SELECT t.*,p.message,p.pid,p.authorid,f.name FROM ".DB::table( "forum_thread" )." t  INNER JOIN ".DB::table( "forum_post" )." p on p.tid=t.tid and p.first=1 INNER JOIN ".DB::table( "forum_forum" )." f on f.fid=t.fid  WHERE t.displayorder>=0 ".$whereadd." and t.tid in (".$tidarray.") group by t.tid ORDER BY t.dateline DESC limit 10" );
		
		$tableid = array();
		
		$instr = getDImplode2($threadlisttags, 'pid');
		...
		unset($tableid);
		unset($threadlisttags);
		$thread['dateline'] =  dgmdate($thread['dateline'], 'u', '9999', getglobal('setting/dateformat').' H:i');
		
		
		DB::query("update  " .DB::table('forum_thread')." set chakan=chakan+1 where  tid=".$thread['tid']);	
		
		$thread['pid'] =  DB::result_first("SELECT pid FROM " .DB::table('forum_post')." where tid=".$thread['tid']." and first=1 ");
		
		
		
		$thread['forum_attachtags'] =  DB::fetch_all("SELECT * FROM " .DB::table('forum_attachment')." where pid=".$thread['pid']."");
		
		
		if($_G['uid']){
				$zanguo=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$thread['tid']." and uid=".$_G['uid']." and type=1 ");
				$bishiguo=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$thread['tid']." and uid=".$_G['uid']." and type=2 ");
				$dingyuele=DB::result_first("SELECT id FROM " .DB::table('froum_dingyue')." where puid=".$thread['authorid']." and uid=".$_G['uid']."");
				$shoucang=DB::result_first("SELECT favid FROM " .DB::table('home_favorite')." where uid=".$_G['uid']." and id=".$thread['tid']." and idtype='tid' ");
		}else{
		
			$zanguo=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$thread['tid']." and openid='".$openid."' and type=1 ");
			$bishiguo=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$thread['tid']." and openid='".$openid."' and type=2 ");
		}
		if($zanguo){
				$zanguo="on";
				$thread['zanguo']="on";
		...
		$thread['message'] =  DB::result_first("SELECT message FROM " .DB::table('forum_post')." where tid=".$thread['tid']." and first=1 ");
		
		$thread['message']=discuzcode($thread['message'],-1, 0, 0, $_G['forum']['allowsmilies'], -1,1,0, 0, 0, $thread['authorid'], "", $thread['pid'], $_G['setting']['lazyload'], $thread['dateline'], 0);
		
		
		$thread['message']=parseattachone( $thread['message'],$thread['pid']);
		
		
		$thread['tags'] =  DB::result_first("SELECT tags FROM " .DB::table('forum_post')." where tid=".$thread['tid']." and first=1 ");
		
		$thread['favtimes']=DB::result_first("SELECT count(favid) FROM " .DB::table('home_favorite')." where id=".$thread['tid']." and idtype='tid' ");
		
		
		$tagarray_all = $posttag_array = array();
			$tagarray_all = explode("\t", $thread['tags']);
			if($tagarray_all) {
				foreach($tagarray_all as $var) {
					if($var) {
						$tag = explode(',', $var);
			...
			$commnetnum = $_G[forum_thread][allreplies];
			
			
			$thread_fx['tid']=$thread['tid'];
							$thread_fx['subject']=$thread['subject'];
		
			
			$tableitem = DB::fetch_all("SELECT aid,pid,tableid FROM %t WHERE pid =".$thread['pid'], array('forum_attachment'));
					
					foreach($tableitem as $k=>$v){
						
							$thread_fx['thumb']=DB::result_first("SELECT attachment FROM " .DB::table('forum_attachment_'.$v['tableid'])." where aid=".$v['aid']." and pid=".$v['pid']." and isimage=1 ");
			...
			if($_G['forum']['fid']==41){
				$tableid = array();
					...
					$tableitem = DB::fetch_all("SELECT aid,pid,tableid FROM %t WHERE pid =".$thread['pid'], array('forum_attachment'));
					$thread['imagelist']=array();
					foreach($tableitem as $k=>$v){
						
							$thread['imagelist'][$k]['image']=DB::result_first("SELECT attachment FROM " .DB::table('forum_attachment_'.$v['tableid'])." where aid=".$v['aid']." and pid=".$v['pid']);
					}
					
				$piclist2 = DB::fetch_all("SELECT tid,subject as title FROM " .DB::table('forum_thread')." where displayorder>=0 and   fid=41 order by dateline desc limit 8");

					foreach($piclist2 as $k=>$v){
						$piclist2[$k]['piclist']=	DB::fetch_all("SELECT aid,tableid,tid FROM " .DB::table('forum_attachment')." where tid=".$v['tid']);
						
						
						
						$piclist2[$k]['picnum']=count($piclist2[$k]['piclist']);
						foreach($piclist2[$k]['piclist'] as $kk=>$vv){
							
								$piclist2[$k]['piclist'][$kk]['attachment']=DB::result_first("SELECT attachment FROM " .DB::table('forum_attachment_'.$vv['tableid'])." where aid=".$vv['aid']);
						}
					}
			...
			$duanzilist = DB::fetch_all( "SELECT t.*,p.message,p.pid,p.authorid,f.name FROM ".DB::table( "forum_thread" )." t  INNER JOIN ".DB::table( "forum_post" )." p on p.tid=t.tid and p.first=1 INNER JOIN ".DB::table( "forum_forum" )." f on f.fid=t.fid  WHERE t.displayorder>=0 and t.fid=42 and t.tid!=".$thread['tid']." group by t.tid ORDER BY t.dateline DESC LIMIT 5" );
			
			foreach($duanzilist as $key=>&$val){
				
				if($_G['uid']){
				$zanguo2=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$val['tid']." and uid=".$_G['uid']." and type=1 ");
				
				}else{
					$ip=$_SERVER["REMOTE_ADDR"]; 	
					$zanguo2=DB::result_first("SELECT id FROM " .DB::table('froum_zan')." where tid=".$val['tid']." and openid='".$openid."' and type=1 ");
			...
			if($_GET['mobile'])
			include template('touch/'.$_G['forum']['vfile']); 
			else
...
function viewthread_numbercard($post) {
	...
	foreach($_G['setting']['numbercard']['row'] as $key) {
		if(substr($key, 0, 10) == 'extcredits') {
			$numbercard[] = array('link' => 'javascript:;', 'value' => $post[$key], 'lang' => $_G['setting']['extcredits'][substr($key, 10)]['title']);
		} else {
...
function getLinkByKey($key, $post, $returnarray = 0) {
	switch($key) {
		...
		case 'credits': $v = array('link' => 'javascript:;', 'value' => $post['credits']);break;
