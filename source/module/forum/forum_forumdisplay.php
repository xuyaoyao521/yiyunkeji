...
if($_G['uid']){
				...
				$shoucang=DB::result_first("SELECT favid FROM " .DB::table('home_favorite')." where uid=".$_G['uid']." and id=".$_GET['fid']." and idtype='fid' ");
		}
...
if($filter !== 'hot') {
	...
	if($filterbool) {
		...
		$filterarr1['displayorder'] = array(2, 4);
		
		$threadlist = C::t('forum_thread')->fetch_all_search2($filterarr1, $tableid, $start_limit, $_G['tpp'], $_order, '');
		unset($filterarr1);
	}
	
	$threadlist = array_merge($threadlist, C::t('forum_thread')->fetch_all_search2($filterarr, $tableid, $start_limit, $_G['tpp'], $_order, '', $indexadd));
