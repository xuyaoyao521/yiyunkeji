...
if($_GET['action'] == 'checkpatch') {
	...
	if($_G['member']['allowadmincp'] == 1) {
		...
		if(!empty($patchnotice['data'])) {
			...
			$patchlist .= '</table><p class="cl"><a href="eyun_admins.php?action=patch" class="y pn"><strong>'.($patchnotice['fixed'] ? $lang['patch_view_fix_detail'] : $lang['patch_fix_right_now']).'</strong></a></p>';
			$patchlist .= '</div></div>';
		}
	}
	...
	if($newversion) {
		...
		echo '<div class="ptn cl"><a href="eyun_admins.php?action=plugins" class="xi2 y">'.$lang['plugin_link'].' &raquo;</a></div>';
		echo '</div></div>';
	}
	include template('common/footer_ajax');
	exit;
