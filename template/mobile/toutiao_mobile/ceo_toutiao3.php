...
$ceo_num = isset($settings['ceo_num']) ? intval($settings['ceo_num']) : 5;
...
if ( $needupdate )
{
	...
	if($ceo_module == 1)
	{
		...
		$whereadd .= " and t.tid != ".$_GET['tid'];
		
			$catid=DB::result_first("SELECT fid FROM " .DB::table('forum_thread')." where tid=".$_GET['tid']);	
			$whereadd .= " and t.fid = ".$catid;
