...
<!--{if !$_G[inajax]}-->
	<div class="pt box_bg"><a href="forum.php">{echo m_lang('home')}</a> <em> &gt; </em> <!--{if $_GET['op'] == 'edit'}-->{lang edit}<!--{elseif $_GET['op'] == 'delete'}-->{lang delete_reply}<!--{/if}--></div>
<!--{/if}-->
...
<div class="bm_c box_bg">
	<form id="deletecommentform_{$cid}" name="deletecommentform_{$cid}" method="post" autocomplete="off" action="home.php?mod=spacecp&ac=comment&op=delete&cid=$cid" {if $_G[inajax]}onsubmit="ajaxpost(this.id, 'return_$_GET[handlekey]');"{/if}>
		...
		<input type="hidden" name="formhash" value="{FORMHASH}" />
		<!--{if $_G[inajax]}--><input type="hidden" name="handlekey" value="$_GET[handlekey]" /><!--{/if}-->
		<div class="mbm mtm xg1">{lang delete_reply_message}</div>
		<button type="submit" name="deletesubmitbtn" value="true" class="btn_default btn_submit"><strong>{lang determine}</strong></button>
	</form>
</div>
	<script type="text/javascript">
		function succeedhandle_$_GET['handlekey'] (url, message, values) {
