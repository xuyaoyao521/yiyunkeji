...
<!-- main postlist start -->
<div class="postlist">
    <h3<!--{if $_GET[page] >= 2}--> class="two"<!--{/if}-->>
...
<ul id="alist">
   ...
   <div class="plc cl<!--{if !$post[first]}--> li_brb<!--{/if}-->" id="pid$post[pid]">
				...
				<li>
                   <span class="avatar"><img src="<!--{if !$post['authorid'] || $post['anonymous']}--><!--{avatar(0, small, true)}--><!--{else}--><!--{avatar($post[authorid], small, true)}--><!--{/if}-->" /></span>
					...
					<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
						<a href="home.php?mod=space&uid=$post[authorid]&do=profile"<!--{if $post[first]}--> class="author"<!--{else}--> class="author2"<!--{/if}-->>$post[author]</a>
					<!--{else}-->
						<!--{if !$post['authorid']}-->
    ...
    <!--{if $_G['forum']['ismoderator']}-->
        <!-- manage start -->
        <!--{if $post[first]}-->
            <!--{if $_G['uid']}--><em><a href="home.php?mod=spacecp&ac=favorite&type=thread&id=$_G[tid]" class="favbtn1 blue">{lang favorite}</a></em><!--{/if}-->
        ...
        <!--{else}-->
            ...
            <div id="moption_$post[pid]" popup="true" class="manages" style="display:none;">
                ...
                <!--{if $_G['group']['allowdelpost']}--><input type="button" value="{lang modmenu_deletepost}" class="dialog btn_default" href="forum.php?mod=topicadmin&action=delpost&fid={$_G[fid]}&tid={$_G[tid]}&operation=&optgroup=&page=&topiclist[]={$post[pid]}"><!--{/if}-->
                <!--{if $_G['group']['allowbanpost']}--><input type="button" value="{lang modmenu_banpost}" class="dialog btn_default" href="forum.php?mod=topicadmin&action=banpost&fid={$_G[fid]}&tid={$_G[tid]}&operation=&optgroup=&page=&topiclist[]={$post[pid]}"><!--{/if}-->
                <!--{if $_G['group']['allowwarnpost']}--><input type="button" value="{lang modmenu_warn}" class="dialog btn_default" href="forum.php?mod=topicadmin&action=warn&fid={$_G[fid]}&tid={$_G[tid]}&operation=&optgroup=&page=&topiclist[]={$post[pid]}"><!--{/if}-->
            </div>
        <!--{/if}-->
        <!-- manage end -->					
    <!--{/if}-->				
...
<!--{/if}-->
   ...
   <!--{if $post['first']}-->
    <div class="titls" id="replay">
    	<dl>
        	<dt>{echo m_lang('thread_reply')}</dt>
            <dd>
    <!--{if $_G['page'] > 1}-->
        <a href="forum.php?mod=viewthread&tid=$_G[tid]&extra=$_GET[extra]">&laquo;{echo m_lang('tthread')}</a>
    <!--{else}-->
        <!--{if $ordertype != 1}-->
...
<!--{if $ceo_norepages > 1}-->    
    ...
    var url = 'forum.php?mod=viewthread&tid=$_G[tid]&extra=$_GET[extra]&ordertype={if $ordertype != 1}2{else}1{/if}&threads=thread';
    var pages=$_G['page'];
...
function setanswer(tid, pid, from){
	if(confirm('{echo m_lang('thread_setanswer')}')){
