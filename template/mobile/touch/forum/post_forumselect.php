...
<link rel="stylesheet" href="template/mobile/toutiao_mobile/css/forum/misc.css" type="text/css" media="all">
<div class="ct">
...
<div class="c forumlistpbl_box">
		<span class="pbnv"><span id="pbnv"></span> <a id="enterbtn" class="xg1" href="javascript:;" onclick="locationforums(currentblock, currentfid)">[{lang nav_forum}]</a></span>
	<ul class="forumlistpbl cl">
		...
		<!--{if $_G['group']['allowpost'] || !$_G['uid']}-->
			<!--{if $special === null}-->
				<button id="postbtn" class="pn xg1" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&mobile=2'" disabled="disabled">{lang send_posts}</button>
			<!--{else}-->
				<button id="postbtn" class="pn xg1" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&special=$special&mobile=2'" disabled="disabled">$actiontitle</button>
			<!--{/if}-->
		<!--{/if}-->
