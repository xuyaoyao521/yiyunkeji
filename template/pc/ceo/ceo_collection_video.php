...
$ceo_num2 = isset($settings['ceo_num']) ? intval($settings['ceo_num']) : 20;
...
if ( $ceo_cachetime )
{
	if ( $_G['timestamp'] - @filemtime( $cachefile ) < $ceo_cachetime )
	{
        if(file_exists($cachefile)){
			$toutiaolist2 = include($cachefile);
    		$allnum2 = intval( $toutiaolist2['allnum'] );
    		unset( $toutiaolist2['allnum'] );
        } else {
            $needupdate = 1;
        }
	} else {
...
if ( $needupdate )
{
	...
	if($ceo_module == 1)
	{
		...
		unset($tableid);
		unset($rs);
		$allnum2 = DB::result_first( "select count(*) from ".DB::table( "home_favorite" ). " h  inner join ".DB::table( "forum_thread" ). "  t on h.id=t.tid where t.displayorder>=0 {$whereadd}" );
	...
	$toutiaolist2['allnum'] = $allnum;
    if($ceo_cachetime > 0)
    {
        @file_put_contents( $cachefile, "<?php\nreturn " . var_export($toutiaolist2, true) . ";" );
    }
    unset( $toutiaolist2['allnum'] );
}
