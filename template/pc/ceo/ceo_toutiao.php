...
if ( $needupdate )
{
	...
	if($ceo_module == 1)
	{
		...
		if(!$_GET['keyword']&&(!$_GET['fid']||$_GET['fid']==38||$_GET['fid']==39)){
				
				$top_list = DB::fetch_all( "SELECT t.tid,t.subject,p.pid FROM ".DB::table( "forum_thread" )." t INNER JOIN ".DB::table( "forum_post" )." p on p.tid=t.tid and p.first=1   WHERE t.displayorder>=0 ".$whereadd." and t.attachment=2 group by tid ORDER BY t.dateline DESC LIMIT 3" );
				$instr = getDImplode($top_list, 'pid');
				...
				foreach($top_list as $k=>$v){
						$notinterested.=",".$v['tid'];
						$top_list[$k]['pic'] = getforumimg($tableid[$v['pid']][1][0], 0, 400, 180, $ceo_pictype);
				}
				
				$notinterested=ltrim($notinterested,",");
		}
		if($notinterested){
				$whereadd .= " and t.tid not in (".$notinterested.")";
				
		}
		$rs = DB::fetch_all( "SELECT t.*,p.message,p.pid,p.authorid,f.name FROM ".DB::table( "forum_thread" )." t  INNER JOIN ".DB::table( "forum_post" )." p on p.tid=t.tid and p.first=1 INNER JOIN ".DB::table( "forum_forum" )." f on f.fid=t.fid  WHERE t.displayorder>=0 ".$whereadd." group by t.tid ORDER BY t.".$ceo_order." DESC LIMIT ".$begin.",".$ceo_num."" );
