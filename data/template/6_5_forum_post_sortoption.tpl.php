...
<?php if(in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'video', 'url', 'range', 'upload', 'range'))) { if($option['type'] == 'calendar') { ?>
...
updatesortattach(aid, url, '<?php echo $_G['setting']['attachurl'];?>forum', '<?php echo $option['identifier'];?>');
                     ...
                     <?php } elseif($option['type'] == 'video') { if(!($option['unchangeable'] && $option['value'])) { ?>
<button type="button" class="pn" onclick="uploadWindow(function (aid, url){sortaid_<?php echo $option['identifier'];?>_upload(aid, url)},'video')"><em><?php if($option['value']) { ?>更新<?php } else { ?>上传<?php } ?></em></button>
<input type="hidden" name="typeoption[<?php echo $option['identifier'];?>][aid]" value="<?php echo $option['value']['aid'];?>" id="sortaid_<?php echo $option['identifier'];?>" />
<input type="text" name="typeoption[<?php echo $option['identifier'];?>]" id="sortaid_<?php echo $option['identifier'];?>_url" value="<?php echo $option['value'];?>" />
...
function sortaid_<?php echo $option['identifier'];?>_upload(aid, url) {
$('sortaid_<?php echo $option['identifier'];?>_url').value = url;
updatesortattach2(aid, url, '<?php echo $_G['setting']['attachurl'];?>forum', '<?php echo $option['identifier'];?>');
}
</script>
<?php } else { ?>
