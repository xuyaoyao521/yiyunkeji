...
0
|| checktplrefresh('./template/mobile/touch/forum/pinglun3.htm', './template/mobile/touch/forum/ceo_relateitems.htm', 1477985170, '3', './data/template/4_3_touch_forum_pinglun3.tpl.php', './template/mobile', 'touch/forum/pinglun3')
|| checktplrefresh('./template/mobile/touch/forum/pinglun3.htm', './template/mobile/touch/forum/forumdisplay_fastpost.htm', 1477985170, '3', './data/template/4_3_touch_forum_pinglun3.tpl.php', './template/mobile', 'touch/forum/pinglun3')
|| checktplrefresh('./template/mobile/touch/forum/pinglun3.htm', './template/mobile/touch/common/seccheck.htm', 1477985170, '3', './data/template/4_3_touch_forum_pinglun3.tpl.php', './template/mobile', 'touch/forum/pinglun3')
;?>
<style>
#side_pr a{ float:right}
.header_bg{ background:#fff; border-bottom:1px solid #e9e9e9}
.header_bg a{ color:#666}

#webtitle{ color:#666; font-size:20px}
</style>
<script>SetWebTitle("详情");SetTopLeftNav("forum_backpage");SetTopfenxiang();$("#top_nav,.toptb").remove();$(".mobile_evaluate_details,.mobile_video_details,.mobile_Satin_list").css("margin-top","45px");$(".header_bg").show();</script>
...
推荐
<?php } else { if(!empty($postno[$post['number']])) { ?><?php echo $postno[$post['number']];?><?php } else { ?> <?php echo $post[number]-1;?><?php echo $postno['0'];?><?php } } ?>
</em>
                    <p>
<?php if($post['authorid'] && $post['username'] && !$post['anonymous']) { ?>
<a href="home.php?mod=auther&amp;uid=<?php echo $post['authorid'];?>"<?php if($post['first']) { ?> class="author"<?php } else { ?> class="author2"<?php } ?>><?php echo $post['author'];?></a>
<?php } else { if(!$post['authorid']) { ?>
<a href="javascript:;">游客 <em><?php echo $post['useip'];?><?php if($post['port']) { ?>:<?php echo $post['port'];?><?php } ?></em></a>
<?php } elseif($post['authorid'] && $post['username'] && $post['anonymous']) { if($_G['forum']['ismoderator']) { ?><a href="home.php?mod=auther&amp;uid=<?php echo $post['authorid'];?>" target="_blank">匿名</a><?php } else { ?>匿名<?php } } else { ?>
...
<?php } ?>
    <?php if($_G['forum']['ismoderator']) { ?>
        <!-- manage start -->
        <?php if($post['first']) { ?>
            <?php if($_G['uid']) { ?><em><a href="home.php?mod=auther&amp;uid=<?php echo $_G['tid'];?>" class="favbtn1 blue">收藏</a></em><?php } ?>
...
<?php } ?>
                ...
                <div class="replybtn">
                    ...
                    <?php } if(($_G['uid']==$post['authorid'])) { ?>
                    <input type="button" value="删除"  onClick="delete_post(<?php echo $post['pid'];?>,<?php echo $post['tid'];?>,this);"  class="btn_default">
...
<a href="javascript:;" onclick="plunzan(<?php echo $post['pid'];?>,this);return false;"><i class="iconfont">&#xf661;</i><?php if($post['postreview']['support']) { ?><span id="review_support_<?php echo $post['pid'];?>"><?php echo $post['postreview']['support'];?></span><?php } ?></a>
                </div>
                <!--div class="replybtn">
...
<?php if($ceo_norepages > 1) { ?>    
    ...
    <?php if($commnetnum > $_G['ppp']) { ?>
<script type="text/javascript">
    var url = 'forum.php?mod=viewthread&tid=<?php echo $_G['tid'];?>&extra=<?php echo $_GET['extra'];?>&ordertype=<?php if($ordertype != 1) { ?>2<?php } else { ?>1<?php } ?>&threads=thread&mobile=2';
    var pages=<?php echo $_G['page'];?>;
    var allpage=<?php echo $thispage = ceil($commnetnum / $_G['ppp']);; ?>;
    </script>
    <?php include template('common/ceo_ajaxpage'); ?> 
    <?php } } else { ?>
    <?php if($multipage) { ?><div class="pgbox"><?php echo $multipage;?></div><?php } } ?>
</div>
<div class="content"><?php $ceo_btoolopen = 0;?><form method="post" autocomplete="off" id="fastpostform" action="forum.php?mod=post&amp;action=reply&amp;fid=<?php echo $_G['fid'];?>&amp;tid=<?php echo $_G['tid'];?>&amp;extra=<?php echo $_GET['extra'];?>&amp;replysubmit=yes&amp;mobile=2">
