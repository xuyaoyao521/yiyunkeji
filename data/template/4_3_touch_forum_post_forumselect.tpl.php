<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('post_forumselect');?><?php include template('common/header'); ?><link rel="stylesheet" href="template/mobile/toutiao_mobile/css/forum/misc.css" type="text/css" media="all">
<div class="ct">
...
<span class="pbnv"><span id="pbnv"></span> <a id="enterbtn" class="xg1" href="javascript:;" onclick="locationforums(currentblock, currentfid)">[进入版块]</a></span>
...
<button id="postbtn" class="pn xg1" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&mobile=2'" disabled="disabled">发新帖</button>
<?php } else { ?>
<button id="postbtn" class="pn xg1" onclick="hideWindow('nav');window.location.href='forum.php?mod=post&action=newthread&fid=' + selectfid + '&special=<?php echo $special;?>&mobile=2'" disabled="disabled"><?php echo $actiontitle;?></button>
<?php } } ?>
