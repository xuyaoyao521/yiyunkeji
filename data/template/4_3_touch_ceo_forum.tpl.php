...
<?php if($catid==42) { ?>
 ...
 <?php if(is_array($toutiaolist)) foreach($toutiaolist as $thread) { ?><div class="mobile_Satin_list">
   ...
   <p><a href="home.php?mod=auther&amp;uid=<?php echo $thread['authorid'];?>&amp;ordertype=1"><img src="uc_server/avatar.php?uid=<?php echo $thread['authorid'];?>&amp;size=middle"><?php echo $thread['author'];?></a><a href="forum.php?mod=viewthread&amp;tid=<?php echo $thread['tid'];?>&amp;ordertype=1"><?php if($thread['replies'] > 0) { ?><?php echo $thread['replies'];?><?php } else { ?>0<?php } ?>评价</a><span class="share"><a href="javascript:;" onClick="sharenv('share_qt');getdata('forum.php?mod=viewthread&tid=<?php echo $thread['tid'];?>&ordertype=1','[段子] - <?php echo $_G['setting']['bbname'];?>','<?php echo $thread['message'];?>');"></a></span><a class="Collection <?php echo $thread['shoucang'];?>"  href="javascript:;" onclick="collection_arc(<?php echo $thread['tid'];?>,this)" >收藏</a></p>
</div>
...
<?php } elseif($catid==40) { ?>
 <?php if(is_array($toutiaolist)) foreach($toutiaolist as $thread) { ?><div class="mobile_public_list video">
        ...
        <div class="title">
            ...
            <p><a href="home.php?mod=auther&amp;uid=<?php echo $thread['authorid'];?>"><img src="uc_server/avatar.php?uid=<?php echo $thread['authorid'];?>&amp;size=middle"><?php echo $thread['author'];?></a><a class="evaluate"><?php if($thread['replies'] > 0) { ?><?php echo $thread['replies'];?><?php } else { ?>0<?php } ?>评价</a><a class="like <?php echo $thread['zanguo'];?>" href="javascript:;" onclick="forumzan(<?php echo $thread['tid'];?>,1,this)"><?php echo $thread['ding'];?></a><span class="share"><a href="javascript:;" onClick="sharenv('share_qt');getdata('forum.php?mod=viewthread&tid=<?php echo $thread['tid'];?>&ordertype=1','[视频] - <?php echo $_G['setting']['bbname'];?>','<?php echo $thread['subject'];?>','<?php echo $thread['pic'];?>');"></a></span></p>
        </div>
</div>
 <?php } ?>
...
<div class="mobile_common_list"> 
    ...
    <ul id="alist">
        <?php $i = 0;?>        <?php if(is_array($toutiaolist)) foreach($toutiaolist as $thread) { ?>            <?php $i++;?>          
            <?php if($thread['attachment'] == 2||$thread['fid']==40) { ?>
    ...
    <?php } else { ?>
    <li class="wmt"><?php echo m_lang('toutiao_nolist'); ?></li>
    <?php } ?>

    <?php if($ceo_norepages > 1) { ?> 
            <div id="ajaxtag"></div>
            <script type="text/javascript">
var url = 'forum.php?mod=phone&mobile=2&<?php echo $ceo_url;?>';
...
var allpage=<?php echo $thispage = ceil($allnum / $ceo_num);; ?>;
 ...
 <?php } ?> 
 <?php include template('common/share'); ?>